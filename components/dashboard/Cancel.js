...
const Cancel = ({navigation}) => {
  ...
  const getCancelWork = () =>{
    axios.post(url, {'vId': global.vId,'workType':'cancel','deviceId': global.deviceId})
    .then(res => {
          if(res.data.error){
      ...
      <ScrollView style={styles.scrollView}>
          <View style={styles.container}>
              ...
              <View style={styles.editPagesMainContainer}>
                {list.length>0?<View>
                  {list.map((list,index) => (
                        <View style={[styles.dashboardDetailBoxMainContainer,{height:205}]} key={index}>
                            ...
                            <View style={[styles.dashboardDetailBoxTwo,{height:40}]}>
                                <View style={[styles.dashboardDetailBoxColumnTwo]}>
                            ...
                            <View style={[styles.dashboardDetailBoxTwo]}>
                                <View style={[styles.dashboardDetailBoxColumnTwo,{width:'100%',height:55,justifyContent:'flex-start'}]}>
                                    <ScrollView nestedScrollEnabled={true} >
                                        ...
                                        <Text style={styles.dashboardDetailBoxColumnDescOne}>{(list.cWork==null?'No Data':list.cWork)}</Text>
                                    </ScrollView>
                                </View>
                            </View>
