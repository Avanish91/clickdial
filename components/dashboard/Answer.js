...
import {View,ScrollView,SafeAreaView,Text,StatusBar,TouchableOpacity,Linking,Image,Share} from 'react-native';
...
const Answer = ({navigation}) => {
    ...
    const getAnswerWork = () =>{
      axios.post(url, {'vId': global.vId,'workType':'answer','deviceId': global.deviceId})
      .then(res => {
            if(res.data.error){
    ...
    <SafeAreaView style={styles.container}>  
        ...
        <ScrollView style={styles.scrollView}>
            <View style={styles.container}>
                ...
                {list.length>0?<View>
                    {list.map((list,index) => (
                        <View style={[styles.dashboardDetailBoxMainContainer,{height:155,flexDirection:'row',flexWrap:'wrap'}]} key={index}>
                            <View style={{width:'80%'}}>
